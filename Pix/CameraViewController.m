...
@interface CameraViewController () <CameraToolBarDelegate, ImageLibraryCollectionViewControllerDelegate>
...
- (void) setupImageCapture {
    ...
    // #2- create a video preview to display the camera content
    ...
    // #3- request permission to use the camera
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
        dispatch_async(dispatch_get_main_queue(), ^{
            // #4- what to do if user says yes/no
            if (granted) {
                // #5- what happens if the user says yes: create the camera
                AVCaptureDevice *device = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
                
                // #6- provides information to the capture session
                NSError *error = nil;
                ...
                if (!input) {
                    ...
                    // #7- add the input to our capture session
            ...
            // #8- if the user says no, tell the user why the program won't work
            else {
...
- (void) rightButtonPressedOnToolbar:(CameraToolbar *)toolbar {
    ...
    ImageLibraryCollectionViewController *imageLibraryCVC = [[ImageLibraryCollectionViewController alloc] init];
    imageLibraryCVC.delegate = self;
    [self.navigationController pushViewController:imageLibraryCVC animated:YES];
}
