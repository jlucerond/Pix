...
//  Blocstagram
//
//  Created by Joe Lucero on 8/18/15.
...
- (void)viewDidLoad {
    ...
    if (CGRectGetHeight(self.view.bounds) > 500) {
        ...
        //this doesn't run in a popover view
        self.navigationItem.rightBarButtonItem = self.sendBarButton;
    }
...
- (void) sendButtonPressed:(id)sender {
    ...
    if ([[UIApplication sharedApplication] canOpenURL:instagramURL]) {
        ...
        [alertVC addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Send", @"Send button") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
         ...
         alertVC = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"No Instagram App", nil) message:NSLocalizedString(@"Download the Instagram App in the App Store.", @"send image instructions") preferredStyle:UIAlertControllerStyleAlert];
